# Viktor Script Language Specification
...
> "Viktor Script must read like a human thought and execute like a machine instruction."

Viktor Script is a foundational, general-purpose programming language built from scratch. It is designed to serve as the base for everything — from operating systems to simple scripts. No dependency. No abstraction borrowed from another language. Everything is built on Viktor Script itself.
...
- Power comes with responsibility — Viktor Script trusts you, not the runtime

---

## 2. File Extension

All Viktor Script source files use the `.vks` extension.
...
These are the atoms of Viktor Script. Everything else is built from these.
...
```vks
let name: str = "Viktor";       // UTF-8, immutable, length-prefixed
...
let buf: byte[] = [86, 105, 107, 116, 111, 114]; // "Viktor" as bytes
```
...
Every Viktor Script program begins execution at the `main` function.
...
let names: byte[][] = ["Viktor", "Atlas", "Callisto"];
...
Viktor Script uses structs instead of classes at the foundation level. No inheritance. No methods inside structs (at this stage). Just data grouped together.
...
let p: Person = Person { name: "Viktor", age: 20 };
...
Viktor Script gives the programmer direct control over memory. This is required for OS-level programming.
...
Every value in Viktor Script has exactly **one owner** at a time. This prevents double-free bugs and makes memory behaviour predictable.
...
This ownership model does not replace `alloc`/`free` at the OS level — those still exist for raw hardware access. But for normal Viktor Script programs, following ownership rules eliminates most memory bugs.
...
Implicit casting does not exist in Viktor Script. All conversions must be explicit.

---

## 14. Error Handling

Functions that can fail return a `Result<T, E>` type. Callers are forced to handle both success and failure. Silent failures do not exist in Viktor Script.
...
Split code across multiple `.vks` files using `import`.
...
function main() {
    let result = add(5, 10); // from math.vks
}
```
...
Viktor Script supports both modes from a single source file.
...
vks compile main.vks -o main.vkb

# Run compiled bytecode
vks exec main.vkb

# Native binary (future — via LLVM target)
vks build main.vks -o main
```
...
*Viktor Script — Built from nothing. Foundation for everything.*
