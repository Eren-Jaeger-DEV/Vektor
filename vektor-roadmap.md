# Vektor Full Roadmap (Phases 1–20)
...
- **Deliverable:** The `--run-ast` command, which allows immediate execution of `.vk` files purely by traversing the AST dynamically in memory.

### Phase 4 — Bytecode Compiler ✅
- **Focus:** Compile AST to optimized bytecode.
- **Deliverable:** Produce `.vkb` (Vektor Bytecode) files using a custom opcode instruction set designed for speed and safety.
...
- **Focus:** Write the Vektor compiler *in* Vektor.
- **Deliverable:** The `vektor-compiler/` directory containing a lexer, parser, and compiler written natively in `.vk`. Bootstrapped using the original TypeScript compiler, proving the language is Turing complete and expressive enough to compile itself.
...
### 9.1 — Status
- `vektor.json` manifest format defined and working
- `vk_modules/` resolution working for the **AST interpreter** (`--run-ast`)
...
### 15.3 — Deliverable
The same `.vk` file produces correct output on all three major OSes, verified automatically, not manually.
...
- `vektor-compiler/pkg.vk` fully replaces `package-manager.ts`
...
### 17.2 — How
Since the compiler is self-hosted and compiles to native code via LLVM, you can compile `vektor-compiler/main.vk` itself, ahead of time, into a native `vks` executable. Ship that binary, not the TypeScript source, to end users.

### 17.3 — Installer scripts
Each script downloads the prebuilt `vks` binary + bundled `runtime` object + standard library `.vk` files, places them in a standard location, and adds `vks` to `PATH`.
...
Real autocomplete, go-to-definition, inline error squiggles. JSON-RPC over stdio; could be written in Vektor itself.

---

## Phase 19 — Battle Test

Build one real, non-toy program entirely in Vektor — ideally something using file I/O, a package dependency, networking, and concurrency. 
