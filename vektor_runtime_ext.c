...
void vk_read_file_ffi(char* p_data, long long p_len, vks_string* out) {
    char* path = (char*)malloc(p_len + 1);
    memcpy(path, p_data, p_len);
    path[p_len] = '\0';
    ...
    out->data = buf;
    out->len = len;
    printf("[DEBUG] vk_read_file_ffi: read %lld bytes, ptr=%p\n", len, buf);
    fflush(stdout);
}
...
void vk_write_file_ffi(char* path_data, long long path_len, char* content_data, long long content_len) {
...
int vk_string_eq(char* l_data, long long l_len, char* r_data, long long r_len) {
    printf("[DEBUG] vk_string_eq(l_ptr=%p, l_len=%lld, r_ptr=%p, r_len=%lld)\n", l_data, l_len, r_data, r_len); fflush(stdout);
